...
3.  **Search some information about `trans` dataframe : number of rows, columns, the types of your data, where they have null values, have a look at their distribution. **

    It is important to know the quality of your data and the definition of the columns when you train a ML model *(missings values, types of columns for exemple).* Details about the dataset, including a dictionary of variables, is available [here](introduction.qmd).
...
::: {.callout-note title="Why extreme values are a problem"}
...
3.  **Handle outlier or erroneous values in the target variable**
    ...
    In the same way, **have a look at the lower distribution to see if there is an issue with low price per square meter**. To do so, plot the distribution of the target y for y below 2000€ per square meter and below 500 € per sqm. 
...
We will set a price ceiling of 100€ to mimic original issue, even if with the synthetic data it's not warranted.
...
**Interquartile Range** method can keep all observations with target value between `D1-1.5*(D9-D1)` and `D9+1.5*(D9-D1)` with D1 and D9 the first and ninth deciles.
...
You will see in the @sec-scikit-learn-pipeline how preprocessing can be performed with scikit-learn.

### Now let's go : prepare the dataframe for the training

1.  Have a look at the list of variables available (called features in machine learning). **Which feature do not bear any additional information and can be dropped ?**
...
-  apply the new function `date_to_days` with [scikit-learn's `FunctionTransformer`](https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.FunctionTransformer.html) ;
...
3.  **Create the Pipeline for training a RandomForestRegressor model.** Go to the [scikit-learn's documentation](https://scikit-learn.org/stable/modules/compose.html#pipelines-and-composite-estimators) on the matter to understand how it works.
