...
So, RF rely on three key elements : building **aggregated regression or classification trees**, with a **randomized subset of features** and a ***bootstrap*** **sample** - a random sample drawn with or without replacement from the original dataset, typically of the same size.
...
For more information, you can read the paragraph on RF hyperparameters [here](https://inseefrlab-github-io.translate.goog/DT_methodes_ensemblistes/chapters/chapter3/2-guide_usage_RF.html?_x_tr_sl=fr&_x_tr_tl=en&_x_tr_hl=en&_x_tr_pto=wapp#sec-hyperparam-rf)

# Exercice 4: Train your first Random Forest model



::: {.callout-important title="Fallback code"}
...
1. **Load the train and test datasets.** For convience and for a start, drop categorical and date feature `prop_type` and `trans_date`. 

::: {.callout-tip collapse="true" title="Hint"}

Remember : in the [Preprocessing section](2-preprocessing.qmd), we created the training sets `X_train` and `y_train`.
...
2.  Using the `RandomForestRegressor` class from the `scikit-learn` package and its [documentation](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestRegressor.html), **define a basic model with the following hyperparameters:**
...
**Note: All parameters of RandomForestRegressor have a default value - you only need to explicitly pass the ones you wish to override.** Have a careful look at the various hyperparameters in the documentation. Do you understand what they do? If no, you can read the paragraph on RF hyperparameters [here](https://inseefrlab-github-io.translate.goog/DT_methodes_ensemblistes/chapters/chapter3/2-guide_usage_RF.html?_x_tr_sl=fr&_x_tr_tl=en&_x_tr_hl=en&_x_tr_pto=wapp#sec-hyperparam-rf).
...
# Train the model on a subsample for faster deployment
...
4.  Print the `Out-of-Bag` error *(the concept of OOB error will be covered in the next exercise)*.
...
5.  Now, **calculate the mean squared error of the model on the test set** - that is, how far off the prediction are from the actual values.

    Note that this is just an illustration of the evaluation process. More details on metrics will be covered in the [dedicated section](4-metrics.qmd).

::: {.callout-tip collapse="true" title="Hint"}

You will need to use the predict method of the model (see the [documentation](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestRegressor.html#sklearn.ensemble.RandomForestRegressor.predict)). You can use the `mean_squared_error` function from the `sklearn.metrics` module. Remember to use the __test set__ for the evaluation, NOT the training set.
...
::: callout-note
For convenience and pedagogical reasons, we are still working on a subset of the features and with the non transformed target. In the real world, we would do this step with the pipeline defined at the end of the [pre-processing part](2-preprocessing.qmd).
:::
...
3.  (*Optional*) Using the *Out-of-Bag (OOB) error*, **plot the convergence of the OOB error as a function `rf_error_oob_plot` of the number of trees**.
...
0.  For convenience, we dropped the features `prop_type`, `trans_date` and trained our model on absolute prices and not on the transformed ones. **Get them back and define the new training and testing datasets `X_train, X_test, y_train, y_test`**.
...
## Part 2 : now let's play : predict the housing price for your custom address {#sec-rf-prediction}
...
Real life prices are far more expensive in Paris :) 
It's a confirmation that the model isn't perfect, as the gradient boosting model. 
**The model would need to be better suited before going to production but it takes more than 2 days to really fine-tune it !**
...
You can now have a look at the [gradient boosting model](3-GB_model.qmd).
Once you have trained both model, you can have a closer look at metrics with the [metrics section](4-metrics.qmd).  
...
# Saving final model to reuse it in the metrics part
from joblib import dump

dump(rf_model_final, "rf_model_final.joblib")
