...
## <i class="bi bi-book"></i> Exercice 4: Train your first Gradient Boosting model

1.  Using the `HistGradientBoostingRegressor` class and its [scikit-learn documentation page](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.HistGradientBoostingRegressor.html#sklearn.ensemble.HistGradientBoostingRegressor), define a basic model with default hyperparameters :

    Train the model with the full pipeline (it should take about less than a minute to train).
...
# create HistGradientBoostingRegressor instance with default hyperparameters
...
2.  Calculate the accuracy of this first model on the test set. What does scikit learn consider as the default score for this type of model ? 
...
#| code-overflow: scroll
#| eval: false
print(f'Default score : {model_gb.score(X_test, y_test):.3f}')  # default is R²
```

3.  In scikit learn's documentation on the [score variable](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.HistGradientBoostingRegressor.html#sklearn.ensemble.HistGradientBoostingRegressor.score), check other types of score scikit learn can perform. Print 3 other metrics. Check that the default score calculated before is the R².

    <details>

    <summary>Click to view a hint</summary>
    The list of metrics should get you [here](https://scikit-learn.org/stable/modules/model_evaluation.html#scoring-string-names) where all the metrics are listed. 
...
print(f'RMSE : {r2_score(y_test, y_pred):.3f}')

```



Congrats 🥳! You have trained your first gradient boosting model. Now, try to find the best hyperparameters to minimize the final predicted error of the model.
